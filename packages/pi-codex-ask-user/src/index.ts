...
const ASK_OVERLAY_WIDTH = '92%' as const
...
type RequestUserInputArgs = Static<typeof requestUserInputSchema>
type RequestUserInputQuestion = RequestUserInputArgs['questions'][number]
...
interface RequestUserInputToolDetails {
...
function contextForQuestion(
  ...
  const progress = total > 1 ? `Question ${index + 1} of ${total}` : 'Question'
  return `${progress} · ${question.header}`
}

function answerFromUIResult(result: AskUIResult): RequestUserInputAnswer {
...
function formatCodexResponseSummary(
  response: RequestUserInputResponse,
): string {
  return Object.entries(response.answers)
    .map(
      ([id, answer]) => `${id}: ${answer.answers.join(', ') || '(no answer)'}`,
    )
    .join('; ')
}
...
async function askCodexQuestion(
  ...
  const options = codexOptionsForQuestion(question)
  const normalizedContext = contextForQuestion(
    question,
    questionIndex,
    questionCount,
  )
    ...
    if (
      ...
      removeOverlayInputListener = ctx.ui.onTerminalInput((data) => {
        ...
        if (nextHidden && !hasAnnouncedHide) {
          ...
          ctx.ui?.notify?.(
            `request_user_input hidden — press ${overlayToggle.spec} to reopen`,
            'info',
          )
        }
        return { consume: true }
...
export default function registerCodexAskUser(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      'Request user input for one to three short questions and wait for the response.',
    promptGuidelines: [
      'Use request_user_input to ask the user for a decision when proceeding would require guessing.',
      'Prefer one question and do not exceed three questions.',
      'Provide 2-3 mutually exclusive options for every question.',
      'Put the recommended option first and suffix its label with "(Recommended)".',
      'Do not include an Other option; the client adds a free-form None of the above option automatically.',
    ],
    ...
    async execute(
      ...
      try {
        for (const [index, question] of params.questions.entries()) {
          ...
          const result = await askCodexQuestion(
            ctx,
            signal,
            question,
            index,
          ...
          response.answers[question.id] = answerFromUIResult(result)
        }
      } catch (error) {
    ...
    renderResult(result, options, theme) {
      const details = result.details as RequestUserInputToolDetails | undefined
      ...
      let text = `${theme.fg('success', '✓ ')}${theme.fg('accent', formatCodexResponseSummary(details.response))}`
      if (options.expanded) {
