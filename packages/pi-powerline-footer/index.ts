// biome-ignore-all lint/suspicious/noControlCharactersInRegex: Terminal key/ANSI parsing intentionally matches control sequences.
// biome-ignore-all lint/suspicious/noExplicitAny: Pi extension APIs expose dynamic runtime shapes.
import {
  existsSync,
  mkdirSync,
  readdirSync,
...
function computeResponsiveLayout(
  ...
  ].filter(isSegmentEnabled)
  const secondaryIds = mergedSegments.secondarySegments.filter(isSegmentEnabled)
...
export default function powerlineFooter(pi: ExtensionAPI) {
  ...
  let _fixedStatusContainer: any = null
  ...
  pi.on('session_start', async (event, ctx) => {
    ...
    const contextWithThinking = ctx as { getThinkingLevel?: () => string }
    getThinkingLevelFn =
      typeof contextWithThinking.getThinkingLevel === 'function'
  ...
  pi.on('before_agent_start', async (event, ctx) => {
    ...
    if (ctx.hasUI && config.workingVibes) {
  ...
  function isStashShortcutInput(data: string): boolean {
    ...
    return (
      ...
      data === '\x1b[27;3;115~' ||
      data === '\x1b[27;3;83~' ||
      matchesKey(data, 'alt+s' as never)
    )
  }
  ...
  pi.registerCommand('powerline', {
    ...
    handler: async (args, ctx) => {
      ...
      const costMatch = /^(?:cost|price)(?:\s+(on|off|toggle))?$/.exec(
        normalizedArgs,
      )
      if (costMatch) {
        ...
        if (
          ...
          ctx.ui.notify(
            `Powerline cost ${config.showCost ? 'enabled' : 'disabled'}`,
            'info',
          )
        } else {
          ctx.ui.notify(
            `Powerline cost ${config.showCost ? 'enabled' : 'disabled'} (not persisted; check settings.json)`,
            'warning',
          )
        }
        return
      ...
      const cacheReadMatch =
        /^(?:cache-read|cache)(?:\s+(on|off|toggle))?$/.exec(normalizedArgs)
      if (cacheReadMatch) {
        const mode = cacheReadMatch[1] ?? 'toggle'
        config.showCacheRead =
          mode === 'toggle' ? !config.showCacheRead : mode === 'on'
        resetLayoutCache()
        requestImmediateStatusRender({ deferDuringTyping: false })
        ...
        if (
          writePowerlineOptionSetting(
            ctx.cwd,
            { showCacheRead: config.showCacheRead },
            config.preset,
          )
        ) {
          ctx.ui.notify(
            `Powerline cache read ${config.showCacheRead ? 'enabled' : 'disabled'}`,
            'info',
          )
        } else {
          ctx.ui.notify(
            `Powerline cache read ${config.showCacheRead ? 'enabled' : 'disabled'} (not persisted; check settings.json)`,
            'warning',
          )
        }
        return
  ...
  pi.registerShortcut(bashModeSettings.toggleShortcut as never, {
  ...
  pi.registerShortcut('alt+s' as never, {
  ...
  pi.registerShortcut(resolvedShortcuts.stashHistory as never, {
    description: 'Open prompt history picker',
    handler: async (ctx) => {
      if (!enabled || !ctx.hasUI) return
      await openStashHistory(ctx)
  ...
  pi.registerShortcut(resolvedShortcuts.copyEditor as never, {
  ...
  pi.registerShortcut(resolvedShortcuts.cutEditor as never, {
  ...
  for (const { shortcutKey, description, action } of CHAT_JUMP_SHORTCUTS) {
    pi.registerShortcut(resolvedShortcuts[shortcutKey] as never, {
      description,
      handler: async (ctx) => {
        if (!enabled || !ctx.hasUI) return
  ...
  pi.registerCommand('vibe', {
    ...
    handler: async (args, ctx) => {
      if (!config.workingVibes) {
        ctx.ui.notify(
          'Working Vibes are disabled by powerline.workingVibes',
          'info',
        )
        return
      }
  ...
  function setupCustomEditor(ctx: any) {
    ...
    const editorFactory = (tui: any, editorTheme: any, keybindings: any) => {
      ...
      editor.handleInput = (data: string) => {
        ...
        attachAutocompleteProvider()
        const followUpText = keybindings.matches(
          data,
          'app.message.followUp' as never,
        )
