// @ts-nocheck
// biome-ignore-all lint/suspicious/noControlCharactersInRegex: Tests intentionally assert terminal escape sequences.
...
afterEach(() => {
  ...
  if (originalGhosttyResourcesDir === undefined)
    delete process.env.GHOSTTY_RESOURCES_DIR
  else process.env.GHOSTTY_RESOURCES_DIR = originalGhosttyResourcesDir
})
...
test('terminal split enables modifyOtherKeys in Ghostty alternate screen', () => {
  const previousTermProgram = process.env.TERM_PROGRAM
  const previousGhosttyResourcesDir = process.env.GHOSTTY_RESOURCES_DIR
  ...
  try {
    ...
    if (previousGhosttyResourcesDir === undefined)
      delete process.env.GHOSTTY_RESOURCES_DIR
    else process.env.GHOSTTY_RESOURCES_DIR = previousGhosttyResourcesDir
  }
})
...
test('terminal split keeps tabbed overlay composition within terminal width', () => {
  ...
  const before = tui.compositeLineAt(
  ...
  compositor.install()
  const after = tui.compositeLineAt(
    `Validation before ${' '.repeat(232)}`,
    overlay,
    20,
    210,
