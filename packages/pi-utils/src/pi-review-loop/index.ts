...
export default function (pi: ExtensionAPI) {
	...
	function updateStatus(ctx: ExtensionContext) {
		if (reviewModeActive) {
			const parts = [
				`Review mode (${currentIteration + 1}/${settings.maxIterations})`,
			];
	...
	pi.on("input", async (event, ctx) => {
		...
		const isTrigger =
			...
			settings.triggerPatterns.some((p) => p.test(event.text));
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		const isTrigger = settings.triggerPatterns.some((p) =>
			p.test(event.prompt),
		);
		if (isTrigger) {
			enterReviewMode(ctx);
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		const hasExitPhrase = settings.exitPatterns.some((p) =>
			p.test(textContent),
		);
	...
	pi.registerCommand("review-start", {
		...
		handler: async (args, ctx) => {
			if (reviewModeActive) {
				...
				activePromptConfig = settings.reviewPromptConfig;
				enterReviewMode(ctx);
				if (freshContext && reviewBoundaryCount < 0)
	...
	pi.registerCommand("review-plan", {
		description:
			"Activate review loop for plans/specs/PRDs with optional custom focus text.",
		handler: async (args, ctx) => {
			if (reviewModeActive) {
				ctx.ui.notify("Review mode is already active", "info");
			} else {
	...
	pi.registerCommand("review-max", {
		...
		handler: async (args, ctx) => {
			...
			ctx.ui.notify(
				`Max review iterations set to ${settings.maxIterations}`,
				"info",
			);
		},
	});
	...
	pi.registerCommand("review-status", {
		...
		handler: async (_args, ctx) => {
			if (reviewModeActive) {
				const parts = [
					`iteration ${currentIteration + 1}/${settings.maxIterations}`,
				];
	...
	pi.registerCommand("review-auto", {
		description:
			"Toggle auto-trigger, or start review with custom focus: /review-auto focus on X",
		handler: async (args, ctx) => {
			const arg = args.trim();
			...
			if (reviewModeActive) {
				ctx.ui.notify(
					`Auto-trigger enabled, focus updated for next iteration`,
					"info",
				);
			} else {
				activePromptConfig = settings.reviewPromptConfig;
				...
				pi.sendUserMessage(buildReviewPrompt(activePromptConfig));
				ctx.ui.notify(
					`Auto-trigger enabled, review started with custom focus`,
					"info",
				);
			}
		},
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			autoTrigger: Type.Optional(
				Type.Boolean({
					description:
						"Enable/disable auto-trigger from keywords (disabled by default)",
				}),
			),
