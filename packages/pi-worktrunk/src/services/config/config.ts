...
export async function createPiWorktreeConfigService() {
  ...
  const store = await createConfigService('pi-worktrunk', {
    defaults: {},
    parse,
    migrations: [
      migration_01,
  ...
  const save = async (data: PiWorktreeConfig) => {
    ...
    if (data.onCreateDisplayOutputMaxLines !== undefined) {
      await store.set(
        ...
        data.onCreateDisplayOutputMaxLines,
        'home',
      );
    }
    ...
    if (data.onCreateCmdDisplayPending !== undefined) {
      await store.set(
        ...
        data.onCreateCmdDisplayPending,
        'home',
      );
    }
    ...
    if (data.onCreateCmdDisplaySuccess !== undefined) {
      await store.set(
        ...
        data.onCreateCmdDisplaySuccess,
        'home',
      );
    }
    ...
    if (data.onCreateCmdDisplayErrorColor !== undefined) {
      await store.set(
        ...
        data.onCreateCmdDisplayErrorColor,
        'home',
      );
    }
  ...
  const current = (ctx: { cwd: string }) => {
    ...
    return {
      ...
      onCreateDisplayOutputMaxLines:
        store.config.onCreateDisplayOutputMaxLines ??
        DEFAULT_ONCREATE_DISPLAY_OUTPUT_MAX_LINES,
      onCreateCmdDisplayPending:
        store.config.onCreateCmdDisplayPending ??
        DEFAULT_ONCREATE_CMD_DISPLAY_PENDING,
      onCreateCmdDisplaySuccess:
        store.config.onCreateCmdDisplaySuccess ??
        DEFAULT_ONCREATE_CMD_DISPLAY_SUCCESS,
      onCreateCmdDisplayError:
        store.config.onCreateCmdDisplayError ??
        DEFAULT_ONCREATE_CMD_DISPLAY_ERROR,
      onCreateCmdDisplayPendingColor:
        store.config.onCreateCmdDisplayPendingColor ??
        DEFAULT_ONCREATE_CMD_DISPLAY_PENDING_COLOR,
      onCreateCmdDisplaySuccessColor:
        store.config.onCreateCmdDisplaySuccessColor ??
        DEFAULT_ONCREATE_CMD_DISPLAY_SUCCESS_COLOR,
      onCreateCmdDisplayErrorColor:
        store.config.onCreateCmdDisplayErrorColor ??
        DEFAULT_ONCREATE_CMD_DISPLAY_ERROR_COLOR,
...
export const DefaultOnCreateDisplayOutputMaxLines =
  DEFAULT_ONCREATE_DISPLAY_OUTPUT_MAX_LINES;
export const DefaultOnCreateCmdDisplayPending =
  DEFAULT_ONCREATE_CMD_DISPLAY_PENDING;
export const DefaultOnCreateCmdDisplaySuccess =
  DEFAULT_ONCREATE_CMD_DISPLAY_SUCCESS;
export const DefaultOnCreateCmdDisplayError =
  DEFAULT_ONCREATE_CMD_DISPLAY_ERROR;
export const DefaultOnCreateCmdDisplayPendingColor =
  DEFAULT_ONCREATE_CMD_DISPLAY_PENDING_COLOR;
export const DefaultOnCreateCmdDisplaySuccessColor =
  DEFAULT_ONCREATE_CMD_DISPLAY_SUCCESS_COLOR;
export const DefaultOnCreateCmdDisplayErrorColor =
  DEFAULT_ONCREATE_CMD_DISPLAY_ERROR_COLOR;
export type PiWorktreeConfigService = Awaited<
  ReturnType<typeof createPiWorktreeConfigService>
>;
export type PiWorktreeConfiguredWorktreeMap =
  PiWorktreeConfigService['worktrees'];
